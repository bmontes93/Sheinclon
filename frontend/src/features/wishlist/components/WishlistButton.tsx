...
const WishlistButton: React.FC<WishlistButtonProps> = ({ productId, size = 'default', showText = true, className = '' }) => {
  ...
  const handleToggleWishlist = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setIsInList(result.isInWishlist || false);
      } else {
        alert(result.error);
      }
    } catch (error) {
  ...
  return (
    <button
      ...
      aria-label={isInList ? 'Quitar de lista de deseos' : 'Agregar a lista de deseos'}
    >
      {loading ? (
